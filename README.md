...
**Hosted deploy:** `render.yaml` is a Render Blueprint (free tier — API + web as
Docker web services; Postgres from Neon, optional Redis from Upstash; the event
worker runs in-process via `NIRIKSHAN_RUN_WORKER_IN_PROCESS`). Step-by-step in
